@import {"GrainSwarm", "GameTrak", "Granulator"}
...
public class Transport
{
    ...
    fun void transport(Granulator grain, float step, int loop)
    {
        1 => int flipFlop;
        while(true)
        {
            ...
            do 
            {
                grain.setPosition(grain.position() + (flipFlop * (step/2.0)));
                250::ms => now;
            } until (grain.getPosition() >= 1.0 || grain.getPosition() <= 0.0);

            if(loop == 2) 
            {
                // if we were going backwards flip around 
                if(grain.getPosition() <= 0.001) { 1 => flipFlop; }
                // else flip the other way
                else if(grain.getPosition() >= 1.0) { -1 => flipFlop; }
            }

            // else if go back to the beginning
            else if(loop == 1)
            {
                grain.getPosition(0.0);
            }
        } 
    }
}